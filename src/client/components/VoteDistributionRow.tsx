...
export default function VoteDistributionRow({ storyPoints, voters, isWinningVote = false }: VoteDistributionRowProps) {
  ...
  return <div {...css(styles.container)} ref={containerRef}>
    <Grid container wrap="nowrap">
      ...
      <Grid item {...css(styles.avatarContainer)} ref={avatarContainerRef}>
        <AvatarGroup {...css(styles.avatarGroup)}>
          ...
          hiddenAvatars.length > 0 && <Tooltip title={hiddenAvatars.map(user => <div key={user.id}>{user.name}</div>)} key="hidden"><Avatar {...css(styles.avatar, styles.overflowAvatar)}>+{hiddenAvatars.length}</Avatar></Tooltip>]}
        </AvatarGroup>
      </Grid>
    </Grid>
  </div>;
